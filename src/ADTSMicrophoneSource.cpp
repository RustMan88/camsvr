...
#include "ADTSMicrophoneSource.hpp"
...
ADTSMicrophoneSource* ADTSMicrophoneSource::createNew(UsageEnvironment& env, const char* device) 
{
  FILE* fid = NULL;
  do {
...
#endif
    return new ADTSMicrophoneSource(env, fid, profile,
...
ADTSMicrophoneSource::ADTSMicrophoneSource(UsageEnvironment& env, ADTSMicrophoneCaptureThread* thread)
  : FramedSource(env), mThread(thread)
{

}

ADTSMicrophoneSource::~ADTSMicrophoneSource() 
{
    mThread->Destroy();
}
...
void ADTSMicrophoneSource::doGetNextFrame() {
